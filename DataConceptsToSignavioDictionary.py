...
options = st.multiselect(label='Select the communities you want to search on', options=sorted([f"{k}" for k,v in communities.items()]))

communitiesToQuery = [communities.get(community) for community in options] if options else st.warning("Please specify.") & st.stop()


# choose the collibra asset types to list
options = st.multiselect(label='Choose the asset types you want to search for', options=sorted([f"{k}" for k,v in assetTypes.items()]))

assetTypesToList = [assetTypes.get(assetType) for assetType in options] if options else st.warning("Please specify.") & st.stop()


# choose the collibra status types to include
options = st.multiselect(label='Choose the status types you want to search for ', options=sorted([f"{k}" for k,v in statuses.items()]))

statusTypesToGet = [statuses.get(status) for status in options] if options else st.warning("Please specify.") & st.stop()


# choose the collibra attribute holding href
option = st.selectbox(label='Choose the attribute holding the signavio href ', options=sorted([f"{k}" for k,v in attributeTypes.items()]), index=None)

attributeTypeToSet = attributeTypes.get(option) if option else st.warning("Please specify.") & st.stop()
...
try:
    with st.spinner():
        _=[assetsToUpdate.append(asset) if "href" in asset else assetsToCreate.append(asset) for asset in getAssets(collibra, communitiesToQuery, assetTypesToList, statusTypesToGet, attributeTypeToSet.get("id"))]
...
signavio = {"host": signavio.get("host"), "tenant": signavio.get("tenant"), "username": signavio.get("username"), "password": signavio.get("password")}
...
option = st.selectbox(label='Tell us what data category you want to register', options=sorted([f"{k}" for k,v in categories.items()]), index=None)

categoryToUpdate = categories.get(option) if option else st.warning("Please specify.") & st.stop()
...
option = st.selectbox(label='Choose your category asset id custom attribute', options=sorted([f"{k}" for k,v in attributes.items()]), index=None)
...
option = st.selectbox(label='Choose your category type id custom attribute', options=sorted([f"{k}" for k,v in attributes.items()]), index=None)
...
option = st.selectbox(label='Choose your category href id custom attribute', options=sorted([f"{k}" for k,v in attributes.items()]), index=None)
